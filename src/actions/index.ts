...
export const server = {
  contactForm: defineAction({
    ...
    input: z.object({
      ...
      phone: z.string().min(10, { message: "Phone number is required and need to be at least 10 digits." }),
    ...
    handler: async (formData) => {
      ...
      if (formData["quantity-wet-food-250"] && formData["quantity-wet-food-250"] > 0) {
      ...
      if (formData["quantity-wet-food-100"] && formData["quantity-wet-food-100"] > 0) {
        selectedProducts.push({
      ...
      if (formData["quantity-treats-150"] && formData["quantity-treats-150"] > 0) {
        selectedProducts.push({
